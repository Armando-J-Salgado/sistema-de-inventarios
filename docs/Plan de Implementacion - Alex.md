...
1. **Escritura de `Stock.quantity`**: la mutan entradas/salidas/transferencias del `MovementService` (Armando). Alex es dueño del **lado lectura**. Confirmar que Movement es el único que escribe, y transaccionalmente.
   - **Dato de Nehemías (dueño de `Lot`, 2026-07-21)**: `Lot.state` tiene 3 valores — `PENDING` (lote registrado, aún no disponible), `RECEIVED` (lote llegó físicamente; desde aquí ya se pueden reclamar SKUs y registrar entradas a stock), `CLOSED` (todos los SKUs del lote ya fueron procesados, no acepta más movimientos). El gate para `MovementService.createEntry` es `Lot.state === 'RECEIVED'`. No confundir con `ACTIVE_RESERVATION_STATUSES` (`src/stocks/constants.ts`, `['PENDING', 'ACTIVA']`) — es un campo distinto (`Reservation.status`, no `Lot.state`), mismo nombre `PENDING` por coincidencia, sin relación entre sí.
...
- **Estados de `Reservation` activos (costura #2)** — decisión provisional tomada para poder implementar: `ACTIVE_RESERVATION_STATUSES = ['PENDING', 'ACTIVA']` en `src/stocks/constants.ts`. Ajustar ese arreglo (un solo lugar) cuando el equipo confirme los valores reales que usará el módulo de Reservations.
